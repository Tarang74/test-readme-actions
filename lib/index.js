...
exports.parseLectureNotesContents = void 0;
...
async function run() {
    try {
        ...
        try {
            CodeOwners = await client.request("GET /repos/{owner}/{repo}/contents/{path}", {
                owner: github_1.context.actor,
                repo: github_1.context.payload.repository.name,
                path: `CODEOWNERS`,
                ref: github_1.context.sha,
        ...
        // UNIT_NAME
        UNIT_NAME = github_1.context.payload.repository.name;
        // WHICH_NOTES
        if (LN && EN) {
        ...
        parseCODEOWNERS(CodeOwners.data.toString(), github_1.context.actor);
        // TODO Add alternative if only exam notes are provided.
        if (LectureNotes == undefined)
            return (0, core_1.error)('Lecture notes returned undefined.');
